defmodule Mix.Tasks.Plsm do
    ...
    def run(_) do
        ...
        |> Enum.filter(fn(table) -> table.name !== "schema_migrations" end)
...
defmodule Mix.Tasks.Plsm.Absinthe do
    ...
    def run(_) do
        ...
        |> Enum.map(fn x -> {x, Plsm.Database.get_columns(x.database, x)} end)
        |> Enum.map(fn {header, columns} -> %Plsm.Database.Table {header: header, columns: columns} end)
        |> Enum.map(fn table -> Plsm.IO.Absinthe.prepare(table, configs.project.name) end)
        |> Enum.map(fn {header, output} -> Plsm.IO.Absinthe.write(output, header.name, configs.project.absinthe_destination) end)
    end
end
