defmodule Plsm.IO.Export do
  ...
  def prepare(table, project_name) do
      ...
      columns_without_id = Enum.filter(table.columns, fn(column) ->
        column.name !== "id"
      end)
      ...
      output = output <> changeset(columns_without_id) <> end_declaration()
  ...
  defp module_declaration(project_name, table_name) do
    ...
    "defmodule #{project_name}.Schemas.#{namespace} do\n"
  end
  ...
  defp changeset(columns) do
    ...
    output = output <> four_space "|> Ecto.Changeset.cast(params, " <> changeset_list(columns) <> ")\n"
  ...
  defp belongs_to_output(project_name, column) do
    ...
    "\n" <> four_space "belongs_to :#{column_name}, #{project_name}.Schemas.#{table_name}"
  end

  defp remove_foreign_keys(columns) do
    Enum.filter(columns, fn(column) ->
      column.foreign_table == nil and column.foreign_field == nil and column.name !== "id"
    end)
  end
end